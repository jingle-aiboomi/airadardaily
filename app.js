...
function indexCompanies(list){
  list.forEach(c => {
    ...
    c._tYear     = new Set(c.founded ? [String(c.founded)] : []);
    c._hay = (c.name + ' ' + c.founders + ' ' + c.sector + ' ' + c.snippet + ' ' + (c.founded || '') + ' ' + c.post).toLowerCase();
  });
}
...
/* Pulls founding-year intent out of a query and returns a predicate plus the
   leftover text, so "healthcare 2023" or "voice agents since 2022" both work. */
const THIS_YEAR = 2026;
function parseYearIntent(raw){
  let rest = ' ' + raw + ' ', pred = null, label = null;
  const take = (m) => { rest = rest.replace(m[0], ' '); };
  let m;
  if ((m = rest.match(/\b(?:pre|before|prior to|earlier than|older than)[\s-]*((?:19|20)\d{2})\b/))){
    const y = +m[1]; pred = v => v < y; label = 'before ' + y; take(m);
  } else if ((m = rest.match(/\b(?:after|post)[\s-]*((?:19|20)\d{2})\b/))){
    const y = +m[1]; pred = v => v > y; label = 'after ' + y; take(m);
  } else if ((m = rest.match(/\b(?:since|from|starting)[\s-]*((?:19|20)\d{2})\b/))
          || (m = rest.match(/\b((?:19|20)\d{2})\s*(?:onwards?|\+)\b/))){
    const y = +m[1]; pred = v => v >= y; label = y + ' onwards'; take(m);
  } else if ((m = rest.match(/\b(?:between\s+)?((?:19|20)\d{2})\s*(?:-|–|—|to|and|until|through)\s*((?:19|20)\d{2})\b/))){
    const a = +m[1], b = +m[2], lo = Math.min(a,b), hi = Math.max(a,b);
    pred = v => v >= lo && v <= hi; label = lo + '–' + hi; take(m);
  } else if ((m = rest.match(/\blast\s+(\d{1,2})\s*(?:years?|yrs?)\b/))){
    const y = THIS_YEAR - (+m[1]); pred = v => v >= y; label = 'last ' + m[1] + ' years'; take(m);
  } else if ((m = rest.match(/\b((?:19|20)\d0)s\b/))){
    const d = +m[1]; pred = v => v >= d && v <= d + 9; label = d + 's'; take(m);
  } else {
    const years = [...rest.matchAll(/\b((?:19|20)\d{2})\b/g)].map(x => +x[1])
                    .filter(y => y >= 1990 && y <= THIS_YEAR + 1);
    if (years.length){
      const set = new Set(years);
      pred = v => set.has(v); label = years.join(', ');
      years.forEach(y => { rest = rest.replace(new RegExp('\\b' + y + '\\b'), ' '); });
    }
  }
  // strip now-orphaned connector words like "founded", "est", "started"
  if (pred) rest = rest.replace(/\b(founded|found|est|established|started|launched|incorporated|in|from|of|the|companies|startups)\b/g, ' ');
  return { pred, label, rest: rest.trim().replace(/\s+/g, ' ') };
}
...
function search(query, list){
  ...
  // founding-year intent filters the pool first, then the rest is searched normally
  const yi = parseYearIntent(raw);
  if (yi.pred){
    list = list.filter(c => c.founded && yi.pred(c.founded));
    if (!yi.rest) return list.slice().sort((a, b) => (b.founded - a.founded) || (b.seq - a.seq))
                             .map(c => ({ c, score: 100 }));
    raw = yi.rest;
  }
...
let lastFocus = null;
const state = { q: '', sector: 'All', era: 'all', sort: 'date-desc', userSort: false };
...
function inEra(c){
  ...
  const y = c.founded;
  if (!y) return false;
  if (e === 'pre2020') return y < 2020;
  if (e.includes('-')){ const [a,b] = e.split('-').map(Number); return y >= a && y <= b; }
  return y === Number(e);
}
...
if (eraEl) eraEl.addEventListener('change', e => { state.era = e.target.value; apply(); });

function apply(){
  const pool = DATA.filter(c => (state.sector === 'All' || c.sector === state.sector) && inEra(c));
...
function render(list){
  grid.innerHTML = list.map(c => `
    <button class="card" data-n="${c.n}">
      ...
      <h4>${esc(c.name)}</h4>
      <div class="tag-line"><span class="tag-sector" data-sector="${esc(c.sector)}">${esc(c.sector)}</span>${c.founded ? `<span class="est">Est. ${c.founded}</span>` : ''}</div>
...
grid.addEventListener('click', e => {
  ...
  document.getElementById('m-meta').innerHTML =
    ...
    `<div class="tag-row"><span class="tag-radar">// RADAR</span><span class="tag-sector" data-sector="${esc(c.sector)}">${esc(c.sector)}</span>${acqBadge(c)}${c.founded ? `<span class="est">Est. ${c.founded}</span>` : ''}</div>`;
...
(() => {
  ...
  function openDetail(c){
    ...
    detail.innerHTML = `
      ...
      <div class="pd-tags"><span class="pi-sector">${esc(c.sector)}</span>${acqTag(c)}${c.founded ? `<span class="est">Est. ${c.founded}</span>` : ''}</div>
...
/* ============================================================
   RADAR CONTACTS — company names sweep in and out of the radar
   ============================================================ */
(() => {
  const box = document.getElementById('radar-contacts');
  if (!box) return;
  const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // four sectors of the dial, so names never stack on top of each other
  const ARCS = [[200, 250], [290, 340], [20, 70], [110, 160]];
  const pick = (a, b) => a + Math.random() * (b - a);

  function place(){
    if (!DATA.length) return;
    box.innerHTML = '';
    // shuffle-pick 4 distinct companies
    const chosen = [];
    const used = new Set();
    while (chosen.length < 4 && used.size < DATA.length){
      const i = Math.floor(Math.random() * DATA.length);
      if (used.has(i)) continue;
      used.add(i); chosen.push(DATA[i]);
    }
    chosen.forEach((c, i) => {
      const ang = pick(ARCS[i][0], ARCS[i][1]) * Math.PI / 180;
      const rad = pick(24, 41);                       // % of the dial radius
      const x = 50 + rad * Math.cos(ang);
      const y = 50 + rad * Math.sin(ang);
      const el = document.createElement('div');
      el.className = 'contact' + (x > 52 ? ' flip' : '');
      el.style.left = x.toFixed(1) + '%';
      el.style.top  = y.toFixed(1) + '%';
      el.innerHTML = `<span class="c-dot"></span><span class="c-name">${
        c.name.replace(/[<>&]/g, s => ({'<':'&lt;','>':'&gt;','&':'&amp;'}[s]))}</span>`;
      box.appendChild(el);
      // stagger the fade-in so they land one after another, like real contacts
      setTimeout(() => el.classList.add('on'), reduced ? 0 : 120 + i * 220);
    });
    if (reduced) return;
    setTimeout(() => box.querySelectorAll('.contact').forEach(el => el.classList.remove('on')), 2400);
  }

  const start = () => { place(); setInterval(place, 3000); };
  // wait for the company data to arrive
  (function ready(n){
    if (DATA.length) return start();
    if (n > 40) return;                                // give up quietly
    setTimeout(() => ready(n + 1), 150);
  })(0);
})();
