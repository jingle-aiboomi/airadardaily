...
if (process.env.SKIP_NOTION_SYNC === 'true') {
  console.log('SKIP_NOTION_SYNC=true — using the committed data/companies.json as-is');
  process.exit(0);
}
...
(async () => {
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      founded: p['Founded']?.number ?? null,
      acquirer: rt(p['Acquirer']).trim(),
      acq_date: rt(p['AcquiredDate']).trim(),
      acq_url: (p['AcquisitionURL']?.url || '').trim(),
